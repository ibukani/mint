...
const LAUNCH_DEBOUNCE: Duration = Duration::from_millis(1_500);
...
fn find_steam_artwork(steam_path: &Path, app_id: u32) -> Option<PathBuf> {
...
fn scan_riot_directory(directory: &Path) -> (Vec<InstalledGame>, Option<String>) {
    let mut games = Vec::new();
    let mut warning = None;
...
fn riot_product_id(metadata_folder: &str) -> &str {
...
fn toggle_allowed(previous: Option<Instant>, now: Instant) -> bool {
    interval_elapsed(previous, now, TOGGLE_DEBOUNCE)
}

fn interval_elapsed(previous: Option<Instant>, now: Instant, interval: Duration) -> bool {
