...
- **クイックキャプチャー (Quick Capture)**: **implemented**。自動保存される下書き、タグ付きメモ、Markdown入力支援とテンプレート、コマンドパレット（`Ctrl/Cmd+K`）、全文検索（`tag:` / `is:pinned` / `is:archived` / `has:attachment`）、アーカイブ整理、Markdown書き出し、添付ファイル、バックアップ、クリップボードコピーに対応。
